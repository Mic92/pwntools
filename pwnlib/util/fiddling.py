...
def unbits(s, endian = 'big'):
    ...
    """
    if endian == 'little':
        u = lambda s: six.int2byte(int(s[::-1], 2))
    elif endian == 'big':
        u = lambda s: six.int2byte(int(s, 2))
    else:
...
def xor(*args, **kwargs):
    ...
    strs = [list(six.iterbytes(s)) for s in strs if s != '']
...
def xor_pair(data, avoid = '\x00\n'):
    ...
    if isinstance(avoid, six.text_type):
        avoid = avoid.encode('utf-8')

    alphabet = list(six.int2byte(n) for n in range(256) if six.int2byte(n) not in avoid)

    res1 = b''
    res2 = b''

    for c1 in six.iterbytes(data):
        if context.randomize:
            random.shuffle(alphabet)
        for c2 in alphabet:
            c3 = six.int2byte(c1 ^ six.byte2int(c2))
            if c3 in alphabet:
                res1 += c2
                res2 += c3
                break
...
def xor_key(data, avoid=b'\x00\n', size=None):
    ...
    columns  = [b''] * size
    for word in words:
        for i,byte in enumerate(six.iterbytes(word)):
            columns[i] += six.int2byte(byte)

    alphabet = list(n for n in range(256) if six.int2byte(n) not in avoid)
    ...
    for column in columns:
        if context.randomize:
            random.shuffle(alphabet)
        for c2 in alphabet:
            if all(c^c2 in alphabet for c in six.iterbytes(column)):
                result += six.int2byte(c2)
                break
        else:
            return None
...
def rol(n, k, word_size = None):
    ...
    if isinstance(n, (six.binary_type, six.text_type, list, tuple)):
...
def isprint(c):
    ...
    t = six.b(string.ascii_letters + string.digits + string.punctuation + ' ')
    return c in set(map(six.int2byte, six.iterbytes(t)))|set(map(six.unichr, six.iterbytes(t)))
...
def _hexiichar(c):
    HEXII = six.b(string.punctuation + string.digits + string.ascii_letters)
    if six.int2byte(c) in HEXII:
        return ".%c " % c
    elif c == 0:
        return "   "
    elif c == 0xff:
...
def update_cyclic_pregenerated(size):
    ...
    while size > len(cyclic_pregen):
        cyclic_pregen += six.int2byte(next(de_bruijn_gen))
...
def hexdump_iter(fd, width=16, skip=True, hexii=False, begin=0, style=None,
    ...
    if not hexii:
        def style_byte(b):
            hbyte = '%02x' % six.byte2int(b)
            abyte = b if isprint(b) else '·'
            if not hasattr(abyte, 'encode'):
                abyte = abyte.decode('utf-8')
            if hbyte in style:
                st = style[hbyte]
            elif isprint(b):
            ...
            if st:
                ...
                abyte = st(abyte)
            return hbyte, abyte
        cache = [style_byte(six.int2byte(b)) for b in range(256)]
    ...
    while True:
        ...
        for i, b in enumerate(six.iterbytes(chunk)):
            if not hexii:
...
def hexdump(s, width=16, skip=True, hexii=False, begin=0,
    ...
    """
    s = packing.flat(s)
    return '\n'.join(hexdump_iter(six.BytesIO(s),
                                  width,
                                  skip,
                                  hexii,
                                  begin,
