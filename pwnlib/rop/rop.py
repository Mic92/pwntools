...
class ROP(object):
    ...
    def __init__(self, elfs, base = None, badchars = b'', **kwargs):
        ...
        if isinstance(elfs, ELF):
            elfs = [elfs]
        elif isinstance(elfs, (six.binary_type, six.text_type)):
            elfs = [ELF(elfs)]
    ...
    def generatePadding(self, offset, count):
        ...
        alphabet = b''.join(six.int2byte(c) for c in six.iterbytes(six.b(string.ascii_lowercase)) if c not in self._badchars)
    ...
    def describe(self, object):
        ...
        if isinstance(object, six.binary_type):
            return repr(object)
    ...
    def build(self, base = None, description = None):
        ...
        for idx, slot in iterable:
            ...
            elif isinstance(slot, (six.binary_type, six.text_type)):
                stack.describe(self.describe(slot))
